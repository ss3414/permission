...
public class ShiroINIUtil {
    ...
    public static void main(String[] args) {
        try {
            ...
            String permitQuery = "query";
            List<String> permitList = new ArrayList<String>();
            permitList.add(permitCreate);
            permitList.add(permitUpdate);
            permitList.add(permitDelete);
            permitList.add(permitQuery);
            ...
            User user3 = new User(3, "user3", "123456"); /* INI中不存在 */
    ...
    private static boolean login(User user) {
        ...
        try {
            /* 将用户的数据token，传递到Realm（INI配置）中进行对比 */
            subject.login(token);
